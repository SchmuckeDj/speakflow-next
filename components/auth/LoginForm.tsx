...
export default function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      localStorage.setItem("sf_user", JSON.stringify({ name: data.user.username, email: data.user.email, level: data.user.level || "B1" }));
  ...
  return (
    <div className="w-full max-w-sm space-y-6">
      <div className="text-center space-y-1">
        ...
        <p className="text-sm text-[var(--color-text-2)]">Inicia sesión para continuar practicando</p>
      </div>
      ...
      <p className="text-center text-sm text-[var(--color-text-2)]">
        ¿No tienes cuenta?{" "}
        <Link href="/register" className="text-[var(--color-acc)] hover:underline font-medium">Regístrate gratis</Link>
      </p>
    </div>
  );
}
