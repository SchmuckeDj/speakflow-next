...
export default function NextLevelScreen() {
  ...
  async function startExam() {
    ...
    try {
      ...
      alert("Error conectando al servidor."); setPhase("ready");
    }
  }
    ...
    <div className="max-w-md mx-auto space-y-6 text-center pt-8">
      ...
      <div>
        ...
        <p className="text-[var(--color-text-2)] mt-2">
          Necesitas <span className="text-[var(--color-acc)] font-bold">{needed.toLocaleString()} XP</span> para desbloquear el examen de <span className="font-medium">{currentLevel}</span> → <span className="font-medium">{Object.keys(XP_TO_UNLOCK)[Object.keys(XP_TO_UNLOCK).indexOf(currentLevel) + 1] ?? "C2"}</span>
        </p>
      </div>
      ...
      <button onClick={() => router.push("/dashboard")}
        className="text-sm text-[var(--color-acc)] hover:underline">← Volver al dashboard</button>
    </div>
  );
  ...
  if (phase === "loading") return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      ...
      <p className="text-sm text-[var(--color-text-2)]">Claude está generando tu examen de nivel {currentLevel}...</p>
    </div>
  );
    ...
    <div className="max-w-md mx-auto space-y-6 text-center pt-8">
      ...
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-lg)] p-5 space-y-3 text-left">
        {[
          ...
          { icon: "🔓", text: `Si pasas, subes a nivel ${currentLevel === "A1" ? "A2" : currentLevel === "A2" ? "B1" : currentLevel === "B1" ? "B2" : currentLevel === "B2" ? "C1" : "C2"} y se desbloquean nuevas palabras` },
      ...
      <button onClick={startExam}
        className="w-full py-4 rounded-[var(--radius-lg)] font-bold text-white transition-all hover:brightness-110 hover:-translate-y-0.5"
        style={{ background: "linear-gradient(135deg, var(--color-acc), var(--color-acc-2))", boxShadow: "0 8px 24px rgba(124,106,255,0.4)" }}>
        Comenzar examen →
      </button>
      <button onClick={() => router.push("/dashboard")}
        className="text-sm text-[var(--color-text-3)] hover:text-[var(--color-text)]">← Volver al dashboard</button>
    </div>
  );
  ...
  if (phase === "result" && result) return (
    <div className="max-w-md mx-auto space-y-6 text-center pt-8">
      ...
      <div>
        ...
        <p className="text-[var(--color-text-2)] mt-2">{result.message}</p>
      </div>
      ...
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-lg)] p-5 space-y-3">
        ...
        <div className="h-2 bg-[var(--color-surface-2)] rounded-full overflow-hidden">
          <div className="h-full rounded-full transition-all duration-700"
            style={{
              width: `${result.score}%`,
              background: result.passed ? "linear-gradient(90deg, #10b981, #34d399)" : "linear-gradient(90deg, #ef4444, #f87171)",
            }} />
        </div>
        <div className="flex justify-between text-xs text-[var(--color-text-3)]">
          <span>Mínimo para pasar: 85%</span>
        ...
        {result.passed && (
          <div className="pt-2 border-t border-[var(--color-border)] text-sm">
            <p className="text-emerald-400 font-semibold">
              {result.old_level} → {result.new_level} desbloqueado 🎉
            </p>
            <p className="text-xs text-[var(--color-text-3)] mt-1">Tu XP se ha reiniciado para el nuevo nivel</p>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        {!result.passed && (
          <button onClick={() => { setPhase("ready"); setResult(null); }}
            className="w-full py-3 rounded-[var(--radius-lg)] border border-[var(--color-border)] text-sm text-[var(--color-text-2)] hover:border-[var(--color-border-2)] transition-all">
            Intentar de nuevo
          </button>
        )}
      </div>
  ...
  if (phase === "exam" && q) return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-medium px-2 py-1 rounded-full bg-[var(--color-acc)]/15 text-[var(--color-acc)]">
          Examen {currentLevel} → {nextLevel}
        </span>
        <span className={`text-sm font-mono font-bold ${timeLeft <= 10 ? "text-red-400 animate-pulse" : "text-[var(--color-text-2)]"}`}>
          ⏱ {timeLeft}s
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {q.options.map((opt, idx) => {
          ...
          let style = "";
          if (!answered) {
            style = "border-[var(--color-border-2)] hover:border-[var(--color-acc)]/60 hover:bg-[var(--color-acc)]/5 cursor-pointer";
          } else if (isCorrect) {
          ...
          return (
            <button key={idx} disabled={answered || submitting}
      ...
      {answers[current] !== null && (
        <div className={`p-3 rounded-[var(--radius-md)] border text-sm ${
      ...
      <div className="flex justify-center gap-1 flex-wrap">
        {questions.map((_, i) => (
          <div key={i} className={`h-1.5 rounded-full transition-all ${
            i < current
              ? (answers[i] === questions[i].correct ? "bg-emerald-400 w-4" : "bg-red-400 w-4")
